      ...
      <div style={{ position: 'fixed', top: '1rem', right: '1rem', zIndex: 9999, display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        {alerts.map(alert => (
          <div
            key={alert.id}
            style={{
              background: alert.type === 'success' ? '#22c55e' : alert.type === 'error' ? '#ef4444' : alert.type === 'warning' ? '#f59e0b' : '#3b82f6',
              color: 'white',
          ...
          >
            <div>
              {alert.title && <div style={{ fontWeight: 600, marginBottom: '0.25rem' }}>{alert.title}</div>}
              <div style={{ fontSize: '0.875rem' }}>{alert.message}</div>
            </div>
            <button onClick={() => removeAlert(alert.id)} style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer', fontSize: '1.25rem', marginLeft: '1rem' }}>×</button>
          </div>
        ))}
      </div>
