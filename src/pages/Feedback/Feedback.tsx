...
const mockFeedbacks: Feedback[] = Array.from({ length: 15 }, (_, i) => ({
  ...
  message: ['Great service overall!', 'App crashes on login', 'Need dark mode', 'GPS tracking is slow', 'Loved the new update!'][i % 5],
...
const statusColors: Record<string, string> = { New: '#f59e0b', Reviewed: '#3b82f6', Resolved: '#10b981' };

const Feedbacks: React.FC = () => {
  const [filter, setFilter] = useState('All');
  const filtered = filter === 'All' ? mockFeedbacks : mockFeedbacks.filter(f => f.status === filter);
        ...
        {filtered.map(fb => (
          <div key={fb.id} className="feedback-card card">
            <div className="feedback-header">
              ...
              <span className="status-badge" style={{ background: statusColors[fb.status] + '20', color: statusColors[fb.status] }}>{fb.status}</span>
            </div>
            <div className="feedback-rating" style={{ color: '#f59e0b' }}>{stars(fb.rating)}</div>
            <div className="feedback-category">{fb.category}</div>
            <p className="feedback-message">"{fb.message}"</p>
            <div className="feedback-footer">
              <span>{fb.date}</span>
              <button className="action-btn action-btn--view" title="Mark Resolved"><CheckCircle size={15} /></button>
            </div>
          </div>
        ))}
      </div>
