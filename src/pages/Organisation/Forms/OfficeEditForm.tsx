...
const OfficeEditForm = ({ organisationData }: OfficeEditFormProps) => {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                setLoading(true);
                const [plansRes, orgTypesRes, regTypesRes, statesRes, statusesRes] =
                    await Promise.all([
            ...
            if (existingState) {
                try {
                    const response = await adminApi.get(
                        `/masters/forms/dropdowns/districts/${existingState}`
                    );
        ...
        <div className="rp-form-card rp-form-wide">
            ...
            <div className="rp-section">
                <div className="rp-section-title">
                    <FiBriefcase className="rp-section-icon" style={{ color: '#3b82f6' }} /> BASIC
                    INFORMATION
                </div>
                <div className="rp-section-body">
                    <div className="rp-grid-4">
                        ...
                        <SelectField
                            ...
                            register={register}
                            errors={errors}
                            options={organisationTypes.map((d) => ({
                                label: d.value,
                                value: d.value,
                            }))}
                        />
